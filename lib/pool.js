...
var pool = module.exports = function pool(options, authorizeFn) {
            ...
            DetectCoinData(function () {
                ...
                OnBlockchainSynced(function () {
                    GetFirstJob(function () {
                        if(typeof options.btStream === "undefined") {
                            SetupBlockPolling();
                            SetupBlockSub();
                        } else {
                            SetupBtStream();
    ...
    function SetupBlockSub(){
        ...
        sock.subscribe(options.zmqPubTopic || 'hashblock');
    ...
    function SetupBtStream(){
        var subSock = zmq.socket('sub');

        if(options.btStream.rawSharedEncryptionKey) {
            var curve = zmq.curveKeypair();
            subSock.curve_server = 0;
            subSock.curve_serverkey = new Buffer(options.btStream.rawSharedEncryptionKey, "hex");
            subSock.curve_publickey = curve.public;
            subSock.curve_secretkey = curve.secret;
        }

        subSock.connect(options.btStream.url);
        subSock.subscribe(options.btStream.topic);

        emitInfoLog('Connected to ZMQ BlockTemplate Stream ' + options.btStream.url + "/" + options.btStream.topic);
        let lastJobRefresh = null;

        function process(json, force) {
            const rpcData = JSON.parse(json);

            ProcessBlockTemplate({ response: rpcData.result }, function(error, rpcData, processedBlock) {
                if(processedBlock) {
                    emitLog('New block notification via BT Stream');
                } else if(error == null && force) {
                    _this.jobManager.updateCurrentJob(rpcData);
                }
            });
        }
        ...
        subSock.on('message', function(topic, flags, message) {
            flags = flags.readUIntLE(0, 4);

            const now = new Date();
            const force = lastJobRefresh == null || (now - lastJobRefresh) >= (options.jobRebroadcastTimeout * 0.9 * 1000);

            if(force) {
                emitLog('Updating transactions & rebroadcasting work');
                lastJobRefresh = now;
            }

            // decompress if necessary
            if(flags & 1) {
                zlib.inflateRaw(message, function (error, decompressed) {
                    if(error == null) {
                        process(decompressed, force);
                    } else {
                        callback(error);
                    }
                });
            } else {
                process(message, force);
            }
        });
    }

    function ProcessBlockTemplate(result, callback, force){
        ...
        _this.daemon.cmd('getblocktemplate',
            ...
            function(result){
                if (result.error){
                    ...
                    ProcessBlockTemplate(result, callback, force);
                }
            }, true
        );
    }
